...
config=conf/ecapa_tdnn_WavLM_frozen_1212.yaml #ecapa_tdnn_WavLM_frozen.yaml
exp_dir=/scratch/users/astar/bmsi/liut1/v2_2409_exp/241212_SpkAug_YourTTS_v1_ECAPA_GLOB_c512_ASTP_emb192_WavLM_L_frozen_num_frms150_aug06_spTrue_saFalse_ArcMargin_intertopk_subcenter_SGD_e60
joint_ft_config=conf/ecapa_tdnn_WavLM_joint_ft_1209.yaml #ecapa_tdnn_WavLM_joint_ft.yaml
joint_ft_exp_dir=/scratch/users/astar/bmsi/liut1/v2_2409_exp/241209_SpkAug_YourTTS_v1_ECAPA_GLOB_c512_ASTP_emb192_WavLM_L_joint_ft_num_frms150_aug06_spTrue_saFalse_ArcMargin_intertopk_subcenter_SGD_epoch20
joint_lmft_config=conf/ecapa_tdnn_WavLM_joint_lmft_1209.yaml #ecapa_tdnn_WavLM_joint_lmft.yaml
joint_lmft_exp_dir=/scratch/users/astar/bmsi/liut1/v2_2409_exp/241209_SpkAug_YourTTS_v1_ECAPA_GLOB_c512_ASTP_emb192_WavLM_L_joint_lmft_num_frms300_aug06_spTrue_saFalse_ArcMargin_intertopk_subcenter_SGD_epoch10
...
if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
  ...
  for dset in SpkAug_yourTTS_v1 vox1; do
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
  ...
  torchrun --master_addr=localhost --master_port=29401 --nnodes=1 --nproc_per_node=$num_gpus \
    wespeaker/bin/train.py --config $config \
      ...
      --num_avg ${num_avg} \
      --data_type "${data_type}" \
      --train_data ${data}/SpkAug_yourTTS_v1/${data_type}.list \
      --train_label ${data}/SpkAug_yourTTS_v1/utt2spk \
